...
- 수정 간격: 1초
- 배열에 중복 값이 있을 수 있음
- 'Find FIRST occurrence' 쿼리가 포함될 수 있음
...
- 원본 배열의 정답 또는 현재 수정된 배열의 정답 중 하나를 맞추면 정답으로 인정
...
- Room 3에서는 실시간 수정이 빈번하므로 빠른 응답이 중요합니다
- 'Find FIRST occurrence' 쿼리의 경우, 중복된 값 중 가장 왼쪽 인덱스를 찾아야 합니다
- 배열이 수정되더라도 항상 정렬 상태를 유지한다는 점을 활용하세요