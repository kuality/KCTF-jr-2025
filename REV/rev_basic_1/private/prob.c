...
void encrypt(char *input, uint8_t *output) {
    int len = strlen(input);
    for (int i = 0; i < len; i++) {
        if (i % 3 == 0) {
            output[i] = (input[i] ^ 0x42) + 3;
        } else if (i % 3 == 1) {
            output[i] = (input[i] ^ 0x37) - 5;
        } else {
            output[i] = (input[i] ^ 0x55) + 7;
        }
    }
}
...
uint32_t calculate_checksum(uint8_t *data, int len) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        checksum += data[i] * (i + 1);
        checksum ^= 0xABCDEF00 >> (i % 8);
    }
    return checksum;
}

uint32_t calculate_cross_hash(char *input) {
    int len = strlen(input);
    uint32_t hash = 0x87654321;
    for (int i = 0; i < len; i++) {
        hash = hash * 33 + input[i];
        hash ^= (hash >> 16);
        hash = ((hash << 3) | (hash >> 29)) + i;
    }
    return hash;
}

int verify(char *input) {
    if (strlen(input) != 19) return 0;
    
    uint8_t target[] = {
        0x1d, 0x09, 0x3f, 0x0c, 0xff, 0x2c, 0x16, 0xfb, 0x2a, 0x0f, 
    ...
    for (int i = 0; i < 19; i++) {
        if (encrypted[i] != target[i]) return 0;
    }
    
    uint32_t checksum = calculate_checksum(encrypted, 19);
    if (checksum != 0x47541429) return 0;
    
    uint32_t cross_hash = calculate_cross_hash(input);
    if (cross_hash != 0x545B24B9) return 0;
